...
- `WORDSTAT_OAUTH_TOKEN` — OAuth-токен Wordstat API (обязательно).
- `WORDSTAT_BASE_URL` — базовый URL Wordstat API (по умолчанию `https://api.wordstat.yandex.net`).
- `WORDSTAT_TIMEOUT_SECONDS` — таймаут API-запроса.
- `REQUEST_LOG_LEVEL` — уровень логирования.
- `DEBUG` — режим отладки.


## Формат JSON-взаимодействия с Wordstat API
Приложение использует Wordstat API по JSON:
- запросы идут по HTTPS методом POST;
- передается `Authorization: Bearer <OAuth-токен>`;
- используется `Content-Type: application/json; charset=utf-8`;
- для подбора ключей используется `/v1/topRequests`, для справочника регионов — `/v1/getRegionsTree`.

Ограничения квот, учтенные в приложении:
- при `429` (персональная квота/лимит RPS) возвращается понятная ошибка;
- если API вернул `Time to refill: N seconds`, пользователю показывается оценка времени ожидания;
- при `503` (общая квота API) возвращается сообщение о временной недоступности сервиса.
...
cp .env.example .env
# заполните WORDSTAT_OAUTH_TOKEN
